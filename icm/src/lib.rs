...
use embassy_stm32::i2c::Error::{Nack};
...
impl<I2C: DroneI2c> Icm20948<I2C, MagDisabled, NotInit>
{
    ...
    */
    ...
    pub async fn initialize_6dof(
        mut self,
    ) -> Result<Icm20948<I2C, MagDisabled, Init>, IcmError> {
    ...
    pub async fn initialize_9dof(mut self) -> Result<Icm20948<I2C, MagEnabled, Init>, IcmError> {
        self.setup_acc_gyr().await?;
    ...
    async fn setup_acc_gyr(&mut self) -> Result<(), IcmError> {
    ...
    async fn setup_mag(&mut self) -> Result<(), IcmError> {
...
impl<I2C, MAG, INIT> Icm20948<I2C, MAG, INIT>
    where
        I2C: DroneI2c,
{
    /// Reset accelerometer / gyroscope module
    pub async fn device_reset(&mut self) -> Result<(), IcmError> {
    ...
    async fn enable_i2c_master(&mut self, enable: bool) -> Result<(), IcmError> {
        self.write_to_flag(Bank0::UserCtrl, u8::from(enable) << 5, 1 << 5).await
    }

    /// Resets I2C master module
    async fn reset_i2c_master(&mut self) -> Result<(), IcmError> {
        self.write_to_flag(Bank0::UserCtrl, 1 << 1, 1 << 1).await
    }

    /// Ensure correct user bank for given register
    async fn set_user_bank<R: Register + Copy>(&mut self, bank: &R, force: bool) -> Result<(), IcmError> {
    ...
    async fn read_from<const N: usize, R: Register + Copy>(&mut self, cmd: R) -> Result<[u8; N], IcmError> {
        let mut buf = [0u8; N];
    ...
    async fn write_to<R: Register + Copy>(&mut self, cmd: R, data: u8) -> Result<(), IcmError> {
        self.set_user_bank(&cmd, false).await?;
    ...
    async fn write_to_flag<R>(&mut self, cmd: R, data: u8, flag: u8) -> Result<(), IcmError>
        where
    ...
    async fn set_mag_read(&mut self) -> Result<(), IcmError> {
    ...
    async fn set_mag_write(&mut self) -> Result<(), IcmError> {
        let [mut reg] = self.read_from(Bank3::I2cSlv0Addr).await?;
        reg &= 0b0111_1111;
        self.write_to(Bank3::I2cSlv0Addr, reg).await
    }
    ...
    async fn mag_write_to(&mut self, reg: u8, data: u8) -> Result<(), IcmError> {
    ...
    async fn mag_read_from<const N: usize>(&mut self, reg: MagBank) -> Result<[u8; N], IcmError> {
        self.set_mag_read().await?;
        Timer::after(Duration::from_millis(10)).await;
    ...
    async fn mag_reset(&mut self) -> Result<(), IcmError> {
    ...
    pub async fn set_acc_range(&mut self, range: AccelerometerRange) -> Result<(), IcmError> {
    ...
    pub async fn set_gyr_range(&mut self, range: GyroscopeRange) -> Result<(), IcmError> {
    ...
    pub async fn set_acc_dlp(&mut self, acc_dlp: AccelerometerDlp) -> Result<(), IcmError> {
    ...
    pub async fn set_gyr_dlp(&mut self, gyr_dlp: GyroscopeDlp) -> Result<(), IcmError> {
    ...
    pub async fn set_acc_odr(&mut self, acc_odr: u16) -> Result<(), IcmError> {
    ...
    /// Set gyroscope output data rate.
    pub async fn set_gyr_odr(&mut self, gyr_odr: u8) -> Result<(), IcmError> {
        self.write_to(Bank2::GyroSmplrtDiv, gyr_odr).await
    }
}

impl<I2C> Icm20948<I2C, MagEnabled, Init>
    where
        I2C: DroneI2c,
{
    ...
    pub async fn read_mag(&mut self) -> Result<Vector3<f32>, IcmError> {
    ...
    pub async fn read_mag_unscaled(&mut self) -> Result<[i16; 3], IcmError> {
    ...
    pub async fn read_all(&mut self) -> Result<Data9Dof, IcmError> {
...
impl<I2C> Icm20948<I2C, MagDisabled, Init>
    where I2C: DroneI2c,
{
    /// Get scaled measurements for accelerometer and gyroscope, and temperature
    pub async fn read_all(&mut self) -> Result<Data6Dof, IcmError> {
...
impl<I2C, MAG> Icm20948<I2C, MAG, Init>
    where
        I2C: DroneI2c,
{
    /// Takes 6 bytes converts them into a Vector3 of floats
    ...
    /// Get array of unscaled accelerometer values
    pub async fn read_acc_unscaled(&mut self) -> Result<Vector3<i16>, IcmError> {
        let raw = self.read_from(Bank0::AccelXoutH).await?;
        Ok(collect_3xi16(raw).into())
    }

    /// Get array of scaled accelerometer values
    pub async fn read_acc(&mut self) -> Result<Vector3<f32>, IcmError> {
        let acc = self
            .read_acc_unscaled()
            .await?
    ...
    pub async fn read_gyr_unscaled(&mut self) -> Result<Vector3<i16>, IcmError> {
        let raw = self.read_from(Bank0::GyroXoutH).await?;
        Ok(collect_3xi16(raw).into())
    }

    /// Get array of scaled gyroscope values
    pub async fn read_gyr(&mut self) -> Result<Vector3<f32>, IcmError> {
        let gyr = self
            .read_gyr_unscaled()
            .await?
    ...
    pub async fn gyr_calibrate(&mut self, num: usize) -> Result<(), IcmError> {
    ...
    async fn set_gyr_offsets(&mut self, offsets: Vector3<i16>) -> Result<(), IcmError> {
